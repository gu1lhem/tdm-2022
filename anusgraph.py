...
def create_database(client):
    print("\n> {0}\n".format(_gremlin_create_database))
    callback = client.submitAsync(_gremlin_create_database)
    if callback.result() is not None:
        ...
        print(
            "Something went wrong with this query: {0}".format(_gremlin_create_database)
        )
    print("\n")
...
_gremlin_create_database = "g.V().drop().iterate(); g.addV('database').property('pk', '{0}').property('name', '{0}')".format(
    COSMODB_DATABASE
)
